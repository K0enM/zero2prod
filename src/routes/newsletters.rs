...
fn basic_authentication(headers: &HeaderMap) -> Result<Credentials, anyhow::Error> {
    let header_value = headers
        ...
        .to_str()
        .context("Authorization header was not an utf8 string")?;
    let base64encoded_segment = header_value
        .strip_prefix("Basic ")
        .context("Authorization scheme was not Basic")?;

    let decoded_bytes = base64::decode_config(base64encoded_segment, base64::STANDARD)
        .context("Failed to base64-decode creds")?;

    let decoded_credentials =
        String::from_utf8(decoded_bytes).context("Decoded credentials are not UTF8")?;
...
async fn validate_credentials(
    ...
    let _users = sqlx::query!(r#"SELECT user_id, username, password_hash FROM users"#)
        .fetch_all(pool)
        .await
        .unwrap();
